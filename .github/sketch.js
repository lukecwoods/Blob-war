...
function setup() {
  ...
  blob[0] = new Blob(socket.id, x, y, 30, r, g, b, 0); // Initialize user blob
...
//function for pushing blobs out


// Function for splitting into multiple pieces if spacebar is pressed
function keyPressed(){
  if (keyCode===32 && blob.length < 25){ //MAX 4 PIECES
      ...
      for (let i = blob.length - 1; i >= 0; i--){
        for (let j = blob.length - 1; j >= 0; j--){
          if (i != j){
            ...
            //fixes merge loop for two blobs
            var other= this.r + this.r;
